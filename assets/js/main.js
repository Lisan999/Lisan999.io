...
document.querySelectorAll('.button-ripple, .search-bar button, .card-btn, .share-form button').forEach(btn => {
...
(function() {
  ...
  if (!canvas || !ctx || !startBtn || !scoreDiv) return;

  const gridSize = 20;
  const tileCount = 16;
  let snake, food, dir, nextDir, score, gameInterval, running;
  ...
  function step() {
    ...
    if (
      ...
      scoreDiv.textContent = '游戏结束！得分：' + score;
      return;
    }
    snake.unshift(head);
...
(function() {
  ...
  if (!snakeCard || !snakeModal || !snakeModalClose) return;
